...
int main()
{
    ...
    for (i = 0; i < LIST_SIZE; i++){
        A[i] = (char *)malloc(sizeof(char) * 33);
        scanf("%s",A[i]);
    }
    ...
    cl_mem a_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, LIST_SIZE * sizeof(char)*33, NULL, &ret);
    ...
    ret = clEnqueueWriteBuffer(command_queue, a_mem_obj, CL_TRUE, 0, LIST_SIZE * sizeof(char)*33, A[0], 0, NULL, NULL);
